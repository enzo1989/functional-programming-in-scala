...
abstract class TweetSet extends TweetSetInterface {
  ...
  def mostRetweetedHelper(curr: Tweet): Tweet
...
case class Empty() extends TweetSet {
  ...
  def mostRetweetedHelper(curr: Tweet): Tweet = curr
...
case class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    val unionRight = right.union(that)
    val unionLeft = left.union(unionRight)
    unionLeft.incl(elem)
  }
  ...
  def mostRetweetedHelper(currTweet: Tweet): Tweet = {
    val bestChildTweet = right.mostRetweetedHelper(left.mostRetweetedHelper(currTweet))
    if(bestChildTweet.retweets > elem.retweets) bestChildTweet else elem
  }
